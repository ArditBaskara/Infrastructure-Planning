...
import bg from "../assets/bg_9.png";

const Vegatasi = () => {
  const [step, setStep] = useState(9);
  const [checkboxStates, setCheckboxStates] = useState(Array(2).fill(false));
      ...
      >
        <div className="w-full h-full flex flex-col justify-start items-center px-4 mt-4">
          ...
          <div className="w-full max-w-lg bg-green-900 p-6 rounded-lg shadow-lg mt-8 sm:mt-6 md:mt-4 lg:mt-4 xl:mt-0 overflow-y-auto max-h-screen">
            <h2 className="text-xl md:text-2xl font-bold mb-4 text-white text-center">Kondisi Vegetasi dan Penggunaan Lahan</h2>
            <form className="space-y-4">
              <div className="space-y-3">
                {[
                  "Apakah wilayah ini memiliki tingkat vegetasi yang cukup baik untuk melindungi dari erosi?",
                  "Apakah penggunaan lahan di wilayah ini sudah sesuai dengan peruntukannya?",
                ].map((question, index) => (
                  <label
                    key={index}
